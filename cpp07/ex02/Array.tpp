...
/*   Updated: 2025/10/02 22:42:54 by mosokina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*The parentheses () (or the function-style cast T()) are the signal 
for Value Initialization.
This guarantees the value is zero for built-in types (like int becomes 0) 
or calls the default constructor for class types.*/
...
Array<T>::Array(unsigned int n): _nSize(n), _array(NULL)
{
	if (_nSize == 0)
		return ;
	_array = new T[_nSize]; // raw memory in heap
	for (unsigned int i = 0; i < _nSize; ++i)
	{
		_array[i] = T(); // value initialisation
	}
